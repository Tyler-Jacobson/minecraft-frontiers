...
StartupEvents.registry("block", event => {
    event.create("frontiers:deer_bait_block")
        ...
        .placementState(placementEvent => {
            ...
            // console.log(`placementEvent.level ${level}`)
            ...
            if (!(level === 'ClientLevel')) {
                level.server.scheduleInTicks(20, () => {
                    try {
                        ...
                        let locationX = pos.x() + 0.5 + Math.cos(angle) * AURA_BLOCK_RADIUS
                        let locationZ = pos.z() + 0.5 + Math.sin(angle) * AURA_BLOCK_RADIUS
                        ...
                        summonHuntableMobDeer(level, aboveGroundTargetDestination)
...
const summonHuntableMobDeer = (level, startingPosition) => {
    // const { level } = mob
    ...
    // huntableMob.setSyncedData('ownerBlock', mob)
    huntableMob.spawn()
}